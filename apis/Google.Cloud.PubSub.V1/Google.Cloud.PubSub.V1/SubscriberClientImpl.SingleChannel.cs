...
public sealed partial class SubscriberClientImpl
{
    ...
    private class SingleChannel
    {
        ...
        private readonly HashSet<HashSet<string>> _messageSetsExpired = new HashSet<HashSet<string>>();
        ...
        private async Task ProcessPullMessagesAsync(List<ReceivedMessage> msgs, HashSet<string> leaseTracking)
        {
            ...
            for (int msgIndex = 0; msgIndex < msgs.Count; msgIndex++)
            {
                ...
                await _taskHelper.ConfigureAwait(_flow.Process(msg.CalculateSize(), _messageOrderingEnabled ? msg.Message.OrderingKey ?? "" : "", async () =>
                {
                    ...
                    // On shutdown we nack everything that we are tracking in either `_messageSetsInLeasing` or
                    // `_messageSetsExpired` and at that point we clear the nacked `leaseTracking` sets.
                    ...
                    lock (leaseTracking)
                    {
                        ...
                        if (leaseTracking.Count == 0)
                        {
                            lock (_lock)
                            {
                                ...
                                _messageSetsExpired.Remove(leaseTracking);
                            }
                        }
                    }
        ...
        private void HandleExtendLease(HashSet<string> leaseTracking, LeaseCancellation cancellation)
        {
            ...
            if (cancellation == null)
            {
                ...
                if (_handlerCts.IsCancellationRequested)
                {
                    ...
                    // Note we can dispose of the registration inmediately here because the token is already cancelled,
                    // so the registered callback will execute inmidiately and synchronously. 
                    using var _ = NackOnShutdownRegistration();
                ...
                else
                {
                    ...
                    Add(_scheduler.Delay(_maxExtensionDuration, cancellation.Token), Next(false, () =>
                    {
                        ...
                        // move these messages to _maxLeaseExpire.
                        ...
                        if (!cancellation.Token.IsCancellationRequested)
                        {
                            lock (leaseTracking)
                            {
                                lock (_lock)
                                {
                                    ...
                                    if (leaseTracking.Count > 0)
                                    {
                                        _messageSetsExpired.Add(leaseTracking);
                                    }
                                }
                            }
        ...
        private CancellationTokenRegistration NackOnShutdownRegistration()
        {
            ...
            void NackAll()
            {
                ...
                lock (_lock)
                {
                    messageSetsToNack = new HashSet<HashSet<string>>(_messageSetsInLeasing.Union(_messageSetsExpired));
                    _messageSetsInLeasing.Clear();
                    _messageSetsExpired.Clear();
                }
